...
struct Five100px {
    ...
    enum Router: URLRequestConvertible {
        ...
        static let consumerKey = "rRrEX6UaoNLfz27NQmSYtWrGk6GHOOkwAJrDauuN"
        ...
        var URLRequest: NSMutableURLRequest {
            let URLPath: (path: String, parameters: [String: AnyObject]) = {
                switch self {
            ...
            let URLRequest = NSURLRequest(URL: URL!.URLByAppendingPathComponent(URLPath.path))
            let encoding = Alamofire.ParameterEncoding.URL
            
            return encoding.encode(URLRequest, parameters: URLPath.parameters).0
        }
    }
